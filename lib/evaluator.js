...
Evaluator.prototype.evaluate = function(xml, options) {
  ...
  var root = $(xml).children('Conditions', 'urn:oasis:names:tc:SAML:2.0:assertion')
    , checks = this._checks
    , errs = []
  ...
  ok = check(root);
  if (ok === undefined) {
    errs.push(new IndeterminateError('SAML conditions have indeterminate attributes'));
  } else if (ok === false) {
    rv = false;
  ...
  for (var cond = conds.first(); cond.length > 0; cond = cond.next()) {
    ...
    if (check) {
      ok = check(cond);
      if (ok === undefined) {
        errs.push(new IndeterminateError('SAML condition is indeterminate: ' + cond.name() + ' ' + cond.ns()));
      } else if (ok === false) {
        rv = false;
      ...
      errs.push(new IndeterminateError('SAML condition not understood: ' + cond.name() + ' ' + cond.ns()));
    }
  }
